...
export default function EnhancedDashboard() {
  ...
  const handleResumeSession = (session: any) => {
    // Navigate to the practice page for the exam this session belongs to
    router.push(`/exam/${session.exam_id}/practice`);
  };

  const confirmDeleteSession = (session: ActiveSession) => {
  ...
  const executeDeleteSession = async () => {
    ...
    try {
      await supabaseExamService.deleteSession(sessionToDelete.id);
      setActiveSessions(prev => prev.filter(s => s.id !== sessionToDelete.id));
      console.log(`Session ${sessionToDelete.id} deleted.`);
