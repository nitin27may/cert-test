...
-- This file is safe to run in Supabase SQL Editor
...
DROP TRIGGER IF EXISTS update_certification_info_updated_at ON certification_info;
CREATE TRIGGER update_certification_info_updated_at BEFORE UPDATE ON certification_info
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_user_exam_sessions_updated_at ON user_exam_sessions;
...
-- Note: You'll need to set up proper admin role management
-- For now, using service_role which should only be used server-side
...
-- Note: In Supabase cloud, these are handled automatically
